...
if {$ICC_REPORTING_EFFORT != "OFF" } {
     ...
     redirect -file $REPORTS_DIR_CLOCK_OPT_CTS/$ICC_CLOCK_OPT_CTS_CEL.clock_timing {report_clock_timing -nosplit -type skew -scenarios [get_scenarios -cts_mode true]} ;# local skew report
     redirect -tee -file $REPORTS_DIR_CLOCK_OPT_CTS/$ICC_CLOCK_OPT_CTS_CEL.max.clock_tree {report_clock_tree -nosplit -summary -scenarios [get_scenarios -cts_mode true]}     ;# global skew report
     }
     if {[llength [get_scenarios -active true -hold true]]} {
     redirect -tee -file $REPORTS_DIR_CLOCK_OPT_CTS/$ICC_CLOCK_OPT_CTS_CEL.min.clock_tree {report_clock_tree -nosplit -summary -scenarios [get_scenarios -active true -hold true]}     ;# min global skew report
     }
}
