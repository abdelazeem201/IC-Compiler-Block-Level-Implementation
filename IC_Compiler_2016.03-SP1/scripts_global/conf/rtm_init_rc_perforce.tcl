## -----------------------------------------------------------------------------
## HEADER $Id: //sps/flow/ds/scripts_global/conf/rtm_init_rc_perforce.tcl#48 $
...
proc rtm_rc_method {} {
  ...
  if { ![info exists ::env(P4PORT)] } {
    return -code error "rtm_rc_method: Perforce selected, but P4PORT not set."
  }
...
proc rtm_rc_file { args } {
  ...
  if { $options(-op) == "co" } {
    ...
    if { [catch { exec p4 edit $real_file } out] } {
...
proc rtm_rc_files_status { args } {
  ...
  foreach line $lines {
    puts $fid $line
    if { [regexp {scripts_global/} $line] } { incr script_flag }
    if { [regexp {scripts_block/}  $line] } { incr script_flag }
  }

  if { $script_flag > 0 } {
    puts $fid "SNPS_ERROR: Scripts open for this Perforce client"
  } else {
    puts $fid "SNPS_INFO: No scripts open for this Perforce client"
  }
...
proc rtm_rc_files_checkin {} {
  ...
  set cmd "xterm -T \"$window_name\" -e $SEV(exec_cmd) -c \"p4 submit; read\""
  catch { eval exec $cmd } out
...
proc rtm_rc_files_update {} {
  ...
  set cmd "xterm -T \"$window_name\" -e $SEV(exec_cmd) -c \"p4 sync; read\""
  catch { eval exec $cmd } out
...
proc rtm_rc_files_checkin_dir { args } {
  ...
  set cmd "find $options(-dir) -type f | xargs p4 add -t text+k"
  catch { eval exec $cmd } out
  puts stderr $out
