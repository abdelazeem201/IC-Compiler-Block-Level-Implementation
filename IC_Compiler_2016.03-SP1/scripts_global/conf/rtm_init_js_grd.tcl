## -----------------------------------------------------------------------------
## HEADER $Id: //sps/flow/ds/scripts_global/conf/rtm_init_js_grd.tcl#85 $
...
proc rtm_job_cmd { args } {
  ...
  set file_part_new [file rootname $file_part_org].rtm_grd_cmd
  set rtm_grd_cmd_file $dir_part_org/$file_part_new
      ...
      grd {
        ...
        if { $options(-aro_enable) } {
          if { $options(-job_misc3) == "-aro_no_opt" } {
            set job_part "$SEV(gscript_dir)/conf/aro_sub_grd -aro_server $SEV(aro_server) -aro_port $SEV(aro_port) -aro_no_opt 1"
          } else {
            set job_part "$SEV(gscript_dir)/conf/aro_sub_grd -aro_server $SEV(aro_server) -aro_port $SEV(aro_port) -aro_no_opt 0"
          }
        } else {
          set job_part "qsub"
        }
        ...
        if { $options(-job_num_cores) > 1 } {
          set job_opts "$job_opts -pe $options(-job_misc4) $options(-job_num_cores)"
        }

        if { $job_res != "" } {
          set job_opts "$job_opts -l '$job_res'"
        }
        ...
        file mkdir [file dirname $rtm_grd_cmd_file]
        set fid [open $rtm_grd_cmd_file w]
        puts $fid $sh_part
        close $fid

        file attributes $rtm_grd_cmd_file -permissions ug+x

        ## Create the command

        set cmd "$job_part -P $queue -N $job_name $job_opts $rtm_grd_cmd_file"
...
define_proc_attributes rtm_job_cmd \
  ...
  {optional value_help {values { grd }}}}
...
proc rtm_kill_cmd { args } {
  ...
  foreach log_file $options(-task_log_file_list) {
    ...
    if { [file exists $jid_file] } {
      ...
      foreach line $lines {
        if { [regexp {^GRD_JOBID\s+(\d+)\s+EOL} $line match jid] } {
