#! /usr/bin/env tclsh
## -----------------------------------------------------------------------------
## HEADER $Id: //sps/flow/ds/scripts_global/conf/wait_for_job_to_finish_grd.tcl#50 $
...
puts "Start of wait_for_job_to_finish_grd.tcl execution."
...
set file_part_new .[file rootname $file_part_org].js_sync.grd
set js_sync_grd $dir_part_org/$file_part_new
...
while { [gets stdin line] >= 0 } {
  ...
  if { [regexp {^Your job (\d+)} $line match job_id] } {
    break
  }
}
...
if { $job_id == "" } {
  ...
  puts "Error: Unable to identify JOBID from qsub results."
  ...
  if { $use_js_sync_function } {
    ...
    set fid [open $js_sync_grd w]
    puts $fid "$js_sync_tcl > $js_sync_log 2>&1"
    close $fid
    file attributes $js_sync_grd -permissions ug+x
    set cmd "qsub -o /dev/null -e /dev/null -cwd -V -l arch=glinux -P $option(queue_name) -N $option(job_name).sync -sync y -hold_jid $job_id $js_sync_grd"
    ...
    set fid [open $js_sync_grd w]
    puts $fid "$js_sync_tcl $js_sync_done > $js_sync_log 2>&1"
    close $fid
    file attributes $js_sync_grd -permissions ug+x
    set cmd "qsub -o /dev/null -e /dev/null -cwd -V -l arch=glinux -P $option(queue_name) -N $option(job_name).sync -sync n -hold_jid $job_id $js_sync_grd"
...
puts "End of wait_for_job_to_finish_grd.tcl execution."
